...
from siamrpn.hrsiam import HRSiamRPNNet
from got10k.datasets import OTB
from siamrpn.dataset import OTBDataset
...
def train(data_dir, net_path=None):
    ...
    seq_dataset_train=OTB(data_dir,version=2015,download=False)
    ...
    train_x_transforms = transforms.Compose([transforms.ToTensor(),
                                             transforms.Normalize(mean=(0.485, 0.456, 0.406),std=(0.229, 0.224, 0.225))])
    """建立训练数据"""
    train_dataset  = OTBDataset(seq_dataset_train,train_z_transforms, train_x_transforms)
    anchors = train_dataset.anchors  #(39015,4)
    """加载训练数据"""
    ...
    """"————————————开始训练——————————————————————"""
    model = HRSiamRPNNet(device=device)
    ...
    for epoch in range(start_epoch, config.epoch + 1):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            #得到模板、搜索图像、目标回归参数、目标分类分数，大小为torch.Size([8,3,127,127])[8,3,255,255][8,39015,4][8,39015]
        ...
        if epoch % config.save_interval == 0:
            if not os.path.exists('./pretrained_otb/'):
                os.makedirs("./pretrained_otb/")
            save_name = "./pretrained_otb/siamrpn_otb_{}.pth".format(epoch)
